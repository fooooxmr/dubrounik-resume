(() => {
  ...
  function createIrbEngine() {
    function buildContext(lang) {
      const roles = lang === 'ru'
        ? [
            ...
            { title: 'Ментор', company: 'IT Academy Grodno', period: '2021–н.в.' },
      ...
      const projects = (typeof PROJECT_KEYS !== 'undefined' ? PROJECT_KEYS : []).map(slug => {
      ...
      const skills = { 'Ruby on Rails': ':expert', Ruby: ':expert', PostgreSQL: ':expert', React: ':advanced', AWS: ':advanced', 'Docker/K8s': ':advanced' };
      ...
      return {
        ...
        Projects: {
          REGISTRY: (typeof PROJECT_KEYS !== 'undefined' ? PROJECT_KEYS : []).map(k => `:${k}`),
          all: () => projects,
          count: () => 9,
          find: name => projects.find(p => p.name.toLowerCase().includes(String(name).toLowerCase().replace(/['"]/g, '')))
        },
        SkillsEngine: { evaluate: () => skills },
        Education: {
          DEGREE: lang === 'ru'
    ...
    function safeEval(expr, ctx) {
      const e = expr.trim();
      if (!e) return null;
      if (e === 'irb') return 'Already in IRB. Type `exit` to return to shell.';
      if (e === 'nil' || e === 'null') return '=> nil';
      if (e === 'true') return '=> true';
      if (e === 'false') return '=> false';
      if (/^\d+(\.\d+)?$/.test(e)) return `=> ${e}`;

      const putsMatch = e.match(/^(puts|p|print)\s+(.+)$/);
      if (putsMatch) {
        const [, , arg] = putsMatch;
        let out;
        if ((arg.startsWith('"') && arg.endsWith('"')) || (arg.startsWith("'") && arg.endsWith("'"))) {
          ...
          try {
            const inner = safeEval(arg, ctx);
            out = inner?.replace(/^=> /, '').replace(/^"(.*)"$/, '$1') ?? arg;
          } catch { out = arg; }
        }
        return { output: out, result: '=> nil' };
      }
      ...
      if (e === 'help' || e === '?') {
        return [
          ...
          '  Experience::ROLES',
          '  Projects.count  /  Projects.find("Avallain")',
  ...
  function handleIrbInput(expr) {
    ...
    try {
      ...
      if (result && typeof result === 'object' && result.output !== undefined) {
        ...
        termPrint(result, 'irb-out');
      }
      if (expr.includes('hire')) setTimeout(showContact, 400);
    } catch (e) {
      termPrint(e.message || String(e), 'error');
    }
    state.irbLine++;
    updateTerminalPrompt();
  }
  ...
  function handleAction(action) {
    ...
    if (action.startsWith('file:')) openFile(action.slice(5));
      ...
      termPrint('Tab renamed (not really — it\'s HTML)', 'info');
      switchToolTab('terminal');
    }
    else if (action === 'extract') {
      termPrint('def excellence; end — saved to contact_controller.rb', 'info');
      switchToolTab('terminal');
    }
  }
